...
//o .replace consegue adicionar as exceções que queremos    
 ...
 var palavra = palavras[i].replace(/[^\wáãàéêíîìïóõôòúüùçñ\s\-]/gi, '').replace(/cadastr(os?|ar?)/gi, 'cadastro').replace(/c[oó]digos?/gi, 'codigo').replace(/carr(inh)?os?/gi, 'carro').replace(/mot(o|inha)s?/gi,'moto').replace(/campinas.+\b/gi,'campinas');
